    ...
    <header className="bg-white border-b border-lbap-border sticky top-0 z-50">
      <div className="container-custom flex items-center justify-between py-4 md:py-6">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                key={link.name}
                to={link.path}
                className={`relative text-[14px] font-medium transition-all py-2 group whitespace-nowrap ${
              ...
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="activeNav"
                    className="absolute -bottom-1 left-0 right-0 h-[2.5px] bg-[#0B96AC]"
        ...
        <div className="hidden xl:block">
          <button className="bg-[#0B96AC] text-white px-6 py-2.5 text-[14px] font-medium shadow-sm hover:bg-[#097b8e] transition-colors rounded-none">
            Request Quote
          </button>
        </div>
