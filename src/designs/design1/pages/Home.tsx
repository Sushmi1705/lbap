...
const Home = () => {
  return (
    <div className="w-full bg-white font-sans text-[#515F78] overflow-hidden">
        ...
        <div className="container-custom">
          ...
          >
            {[
              ...
              <motion.div key={idx} variants={fadeInUp} className="bg-white border border-[#E2E8F0] p-10 hover:shadow-lg transition-all duration-300">
                <div className="w-10 h-10 flex items-center justify-center mb-6">
                  <card.icon className="text-[#0B96AC] w-6 h-6" />
                </div>
                <h3 className="text-[20px] font-bold text-[#191C1E] mb-4">{card.title}</h3>
                {card.desc ? (
                  <p className="text-[13px] leading-relaxed text-[#515F78] font-light">{card.desc}</p>
                ) : (
                  <ul className="space-y-2">
                    {card.list?.map(v => (
                      <li key={v} className="text-[13px] text-[#515F78] font-light flex items-center gap-2">
                        <span className="w-1 h-1 bg-[#0B96AC] rounded-full"></span> {v}
                      </li>
                    ))}
                  </ul>
                )}
      ...
      <section className="section-padding bg-[#F8F9FA]">
        <div className="container-custom">
          ...
          >
            ...
            <p className="text-[14px] leading-relaxed text-[#515F78] font-light max-w-5xl">
      ...
      <section className="bg-[#0B96AC] py-24">
        <div className="container-custom text-center">
          <motion.h2 
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-[36px] font-bold text-white mb-4 tracking-tight"
          >
            Ready to optimize your production?
          </motion.h2>
          <p className="text-white/80 text-[15px] mb-12 max-w-2xl mx-auto font-light">
            Manufacturer of Automotive Fabrication Components... Don't hesitate to contact us for details.
          </p>
          <div className="flex flex-wrap justify-center gap-6">
            <button className="bg-white text-[#0B96AC] px-10 py-3.5 text-[14px] font-medium transition-all hover:bg-gray-50 rounded-none">
              Contact Us
            </button>
            <button className="border border-white text-white px-10 py-3.5 text-[14px] font-medium hover:bg-white hover:text-[#0B96AC] transition-all rounded-none">
              More Details
            </button>
          </div>
        </div>
